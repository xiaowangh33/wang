...
public:
     ...
     */
    RobotInterface(const std::string& robot_name, int dof_num)
        : robot_name_(robot_name),
          dof_num_(dof_num),
          actuator_modes_(dof_num, robot_model::ActuatorMode::PositionPD),
          actuator_safe_modes_(dof_num, robot_model::SafeModeBehavior::LegDamping),
          actuator_ctrl_indices_(dof_num, 0) {
        for (int i = 0; i < dof_num_ && i < robot_model::kTotalDof; ++i) {
            actuator_modes_[i] = robot_model::kJoints[i].mode;
            actuator_safe_modes_[i] = robot_model::kJoints[i].safe;
            actuator_ctrl_indices_[i] = robot_model::kJoints[i].control_index;
        }
    }
    ...
    std::vector<robot_model::ActuatorMode> actuator_modes_;
    std::vector<robot_model::SafeModeBehavior> actuator_safe_modes_;
    std::vector<int> actuator_ctrl_indices_;
     ...
     * @param  input       a dof_num*5 matrix and each column represent kp, goal_joint_pos, kd, goal_joint_vel, torque_feedforward
     */
    ...
    /**
     * @brief True after the concrete hardware layer has permanently latched a
     * runtime safety fault. Simulators and interfaces without such a latch use
     * the safe default false implementation.
     */
    virtual bool IsRuntimeSafetyLatched() const {
        return false;
    }
