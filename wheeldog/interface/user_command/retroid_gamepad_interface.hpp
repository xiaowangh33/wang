...
private:
    ...
    std::atomic<bool> transform_cmd_flag_{false};
...
public:
    ...
    ~RetroidGamepadInterface() override {
        Stop();
    }
    virtual void Start(){
        if(transform_thread_.joinable()) return;
        transform_cmd_flag_.store(true);
    ...
    virtual void Stop(){
        transform_cmd_flag_.store(false);
...
void RetroidGamepadInterface::TransformRetroidToUserCommand(){
    while (transform_cmd_flag_.load()) {
