...
RS01 motion wheel target:       |dq_des| <= 8.0 rad/s
...
独立 `wd_mcu_dryrun.py` 为了单电机台架安全，仍把轮虚拟目标限制为
`8.0 rad/s`；正式部署传输接受 RS01 协议的 `+/-44 rad/s` 虚拟目标。
正式部署已取消轮子的 `8 -> 10 rad/s` 实测速降额/锁停以及 `600 Nm/s`
同向增扭斜率；轮目标只受RS01协议 `+/-44 rad/s` 和17 Nm峰值限制。
...
当前 payload/packet 为 `1104/1120 B`，双MCU 200 Hz上行约 `448.0 kB/s`。
